...
def handle_ip_block(error_text=""):
    """Wird bei 403 'ip not allowed' aufgerufen — setzt Backoff-Timer."""
    ...
    with _ip_block_lock:
        _ip_block_count += 1
        wait = min(IP_BLOCK_BASE * _ip_block_count, IP_BLOCK_MAX)
        _ip_block_until = time.time() + wait
        log(f"⛔ IP BLOCK #{_ip_block_count} erkannt — pausiere {wait}s bis {datetime.fromtimestamp(_ip_block_until).strftime('%H:%M:%S')}")
    return True
