...
# ?? SETTINGS ??????????????????????????????????????????????????????????????????
DB_NAME          = os.getenv("DB_PATH", "/data/trader.db")
# Falls /data nicht existiert (kein Volume), fallback auf lokale DB
import pathlib
try:
...
PERP_ALLOC       = 0.80   # 80% des Perp-Guthabens nutzen
PERP_STOP_PCT    = 0.015
PERP_TP_PCT      = 0.060
...
# ── RATE LIMITER ────────────────────────────────────────────────
def rate_limit():
    wait_if_ip_blocked()   # IP-Block zuerst abwarten
    with _rate_lock:
        now = time.time()
...
# ── IP BLOCK HANDLER ────────────────────────────────────────────
_ip_block_until  = 0.0   # timestamp bis wann geblockt
_ip_block_count  = 0     # wiederholte Blocks zählen
_ip_block_lock   = threading.Lock()
IP_BLOCK_BASE    = 90    # Basis-Wartezeit in Sekunden
IP_BLOCK_MAX     = 600   # max. 10 Minuten warten

def is_ip_blocked():
    """Prüft ob aktuell ein IP-Block aktiv ist."""
    return time.time() < _ip_block_until

def handle_ip_block(error_text=""):
    """Wird bei 403 'ip not allowed' aufgerufen — setzt fixen 120s Timer.
    Counter wird NICHT erhöht — jeder Block = 120s Pause, dann neu versuchen.
    """
    global _ip_block_until, _ip_block_count
    if "ip not allowed" not in error_text.lower() and "ip not allowed" not in str(error_text).lower():
        return False  # kein IP-Block, anderer 403-Fehler
    with _ip_block_lock:
        # Nur setzen wenn kein Block bereits aktiv — kein Hochzählen!
        if time.time() >= _ip_block_until:
            _ip_block_count += 1
            _ip_block_until = time.time() + IP_BLOCK_BASE
            log(f"⛔ IP BLOCK #{_ip_block_count} erkannt — pausiere {IP_BLOCK_BASE}s bis {datetime.fromtimestamp(_ip_block_until).strftime('%H:%M:%S')}")
        # else: Block bereits aktiv, nicht nochmal verlängern
    return True

def reset_ip_block():
    """Nach erfolgreichem API-Call: Block-Counter zurücksetzen."""
    global _ip_block_count, _ip_block_until
    if _ip_block_count > 0:
        with _ip_block_lock:
            log(f"✅ IP-Block aufgehoben nach {_ip_block_count} Block(s)")
            _ip_block_count = 0
            _ip_block_until = 0.0

def wait_if_ip_blocked():
    """Blockiert den aufrufenden Thread bis IP-Block abgelaufen ist."""
    remaining = _ip_block_until - time.time()
    if remaining > 0:
        if remaining > 10:  # Nur loggen wenn > 10s übrig
            log(f"⏳ IP-Block aktiv — warte noch {remaining:.0f}s ...")
        time.sleep(min(remaining + 1, IP_BLOCK_BASE))
        # Nach Warten: Counter zurücksetzen für sauberen Neustart
        with _ip_block_lock:
            if time.time() >= _ip_block_until:
                _ip_block_count = 0
...
def get_candles(client, pair):
    ...
    for attempt in range(2):
        try:
            ...
            e_str = str(e)
            if "403" in e_str and "ip not allowed" in e_str.lower():
                handle_ip_block(e_str)
                return cached[1] if cached else None
            if "429" in e_str and attempt == 0:
...
# ══════════════════════════════════════════════════════════════════════════════
# SIDEWAYS LEARNING ENGINE v1.0
# 3 Komponenten:
#   1) STATISTIK: trackt jeden Trade im sideways-Markt
#   2) RANGE-DETEKTOR: erkennt ob Coin in einer Range ist
#   3) CLAUDE-AGENT: analysiert sideways-Stats und passt Parameter an
# ══════════════════════════════════════════════════════════════════════════════

_sw_params = {
    "min_score":     MIN_SCORE,        # adaptiv anpassbar
    "tp_pct":        LONG_TP_PCT,      # adaptiv anpassbar
    "stop_pct":      LONG_STOP_PCT,    # adaptiv anpassbar
    "range_buy_pct": 0.03,             # kaufe wenn Preis < Range-Low + 3%
    "range_sell_pct":0.03,             # verkaufe wenn Preis > Range-High - 3%
}
_sw_last_claude  = 0
_sw_claude_interval = 3600 * 4   # alle 4h Claude-Agent aufrufen
_range_cache     = {}             # pair -> (ts, range_low, range_high, range_width)
RANGE_CACHE_TTL  = 300            # 5min

def init_sideways_db():
    """Erstellt sideways_trades Tabelle falls nicht vorhanden."""
    try:
        conn = sqlite3.connect(DB_NAME)
        conn.execute("""CREATE TABLE IF NOT EXISTS sideways_trades(
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            pair TEXT, entry REAL, exit_price REAL,
            pnl_pct REAL, profit_usdc REAL,
            phase TEXT, strategy TEXT,
            rsi_entry REAL, bb_width REAL,
            range_low REAL, range_high REAL,
            was_range_trade INTEGER DEFAULT 0,
            created_at TEXT)""")
        conn.execute("""CREATE TABLE IF NOT EXISTS sideways_params(
            key TEXT PRIMARY KEY, value REAL, updated_at TEXT)""")
        conn.commit(); conn.close()
    except Exception as e:
        log(f"SW DB INIT ERR: {e}")

def save_sideways_trade(pair, entry, exit_price, pnl_pct, profit_usdc,
                         phase, strategy, rsi_entry, bb_width,
                         range_low=0, range_high=0, was_range_trade=0):
    """Speichert einen abgeschlossenen Trade mit Sideways-Kontext."""
    try:
        conn = sqlite3.connect(DB_NAME)
        conn.execute("""INSERT INTO sideways_trades
            (pair,entry,exit_price,pnl_pct,profit_usdc,phase,strategy,
             rsi_entry,bb_width,range_low,range_high,was_range_trade,created_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (pair, entry, exit_price, pnl_pct, profit_usdc, phase, strategy,
             rsi_entry, bb_width, range_low, range_high, was_range_trade,
             datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit(); conn.close()
    except Exception as e:
        log(f"SW SAVE ERR: {e}")

def load_sideways_stats(limit=100):
    """Lädt die letzten N Sideways-Trades für Analyse."""
    try:
        conn = sqlite3.connect(DB_NAME)
        rows = conn.execute("""
            SELECT pair, pnl_pct, profit_usdc, strategy, rsi_entry,
                   bb_width, was_range_trade, phase
            FROM sideways_trades
            WHERE phase='sideways'
            ORDER BY id DESC LIMIT ?""", (limit,)).fetchall()
        conn.close()
        return [{"pair":r[0],"pnl":r[1],"profit":r[2],"strategy":r[3],
                 "rsi":r[4],"bb_width":r[5],"range_trade":r[6],"phase":r[7]}
                for r in rows]
    except:
        return []

def save_sw_params():
    """Speichert angepasste Sideways-Parameter in DB."""
    try:
        conn = sqlite3.connect(DB_NAME)
        conn.execute("""CREATE TABLE IF NOT EXISTS sideways_params(
            key TEXT PRIMARY KEY, value REAL, updated_at TEXT)""")
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        for k, v in _sw_params.items():
            conn.execute("INSERT OR REPLACE INTO sideways_params VALUES (?,?,?)",
                         (k, float(v), now_str))
        conn.commit(); conn.close()
    except Exception as e:
        log(f"SW PARAM SAVE ERR: {e}")

def load_sw_params():
    """Lädt gespeicherte Sideways-Parameter aus DB."""
    global _sw_params
    try:
        conn = sqlite3.connect(DB_NAME)
        conn.execute("""CREATE TABLE IF NOT EXISTS sideways_params(
            key TEXT PRIMARY KEY, value REAL, updated_at TEXT)""")
        rows = conn.execute("SELECT key, value FROM sideways_params").fetchall()
        conn.close()
        for k, v in rows:
            if k in _sw_params:
                _sw_params[k] = v
        if rows:
            log(f"SW PARAMS geladen: {_sw_params}")
    except Exception as e:
        log(f"SW PARAM LOAD ERR: {e}")

# ── RANGE DETEKTOR ─────────────────────────────────────────────────────────────
def detect_range(data, pair=""):
    """
    Erkennt ob ein Coin in einer definierten Range konsolidiert.
    Gibt (range_low, range_high, range_width, in_range) zurück.
    - Bollinger Band Breite < 4% = enger Range
    - Preis bewegt sich in einer Box (High-Low der letzten 20 Candles < 5%)
    """
    now = time.time()
    cached = _range_cache.get(pair)
    if cached and now - cached[0] < RANGE_CACHE_TTL:
    ...
    try:
        ...
        upper, lower, basis, bb_width = bollinger(closes)
        if bb_width is None:
            return 0, 0, 0, False

        # Range Box: letzten 20 Candles
        recent_highs = highs[-20:]
        recent_lows  = lows[-20:]
        box_high = max(recent_highs)
        box_low  = min(recent_lows)
        box_width = (box_high - box_low) / box_low if box_low > 0 else 0

        # Range erkannt wenn:
        # - BB-Breite < 4% (komprimiert)
        # - Box-Breite < 6% (enge Preisbewegung)
        in_range = bb_width < 0.04 and box_width < 0.06

        if pair:
            _range_cache[pair] = (now, box_low, box_high, box_width, in_range)

        return box_low, box_high, box_width, in_range
    except Exception as e:
        return 0, 0, 0, False

def is_range_buy(data, pair="", phase="sideways"):
    """
    Range-Trading Entry: Kaufe wenn Preis nahe Range-Low ist.
    Nur im Sideways-Markt sinnvoll.
    Gibt (should_buy, range_low, range_high) zurück.
    """
    if phase != "sideways":
        return False, 0, 0

    range_low, range_high, range_width, in_range = detect_range(data, pair)
    if not in_range or range_low <= 0:
        return False, 0, 0

    closes = data["close"]
    current_price = closes[-1]
    buy_zone = range_low * (1 + _sw_params["range_buy_pct"])

    if current_price <= buy_zone:
        return True, range_low, range_high
    return False, range_low, range_high

def range_tp(entry_price, range_high):
    """Berechnet Range-TP: 80% des Weges zur Range-Obergrenze."""
    if range_high <= entry_price:
        return entry_price * (1 + LONG_TP_PCT)
    target = entry_price + (range_high - entry_price) * 0.80
    return target

# ── STATISTIK ENGINE ───────────────────────────────────────────────────────────
def get_sideways_stats():
    """Berechnet Statistiken aus den gespeicherten Sideways-Trades."""
    trades = load_sideways_stats(100)
    if not trades:
        return None
    ...
    range_trades = [t for t in trades if t["range_trade"]]
    range_wr = (len([t for t in range_trades if t["profit"] > 0]) / len(range_trades)
                if range_trades else 0)

    # Welcher RSI-Bereich funktioniert am besten im Sideways?
    rsi_win_avg  = (sum(t["rsi"] for t in wins if t["rsi"])  / len([t for t in wins if t["rsi"]])
                   if any(t["rsi"] for t in wins) else 50)
    rsi_loss_avg = (sum(t["rsi"] for t in losses if t["rsi"]) / len([t for t in losses if t["rsi"]])
                   if any(t["rsi"] for t in losses) else 60)

    # BB-Breite bei Gewinnen vs Verlusten
    bb_win_avg  = (sum(t["bb_width"] for t in wins if t["bb_width"])  / len([t for t in wins if t["bb_width"]])
                  if any(t["bb_width"] for t in wins) else 0.03)
    bb_loss_avg = (sum(t["bb_width"] for t in losses if t["bb_width"]) / len([t for t in losses if t["bb_width"]])
                  if any(t["bb_width"] for t in losses) else 0.04)

    return {
        "total": total, "wr": wr, "wins": len(wins), "losses": len(losses),
        "avg_win": avg_win, "avg_loss": avg_loss,
        "range_trades": len(range_trades), "range_wr": range_wr,
        "rsi_win_avg": rsi_win_avg, "rsi_loss_avg": rsi_loss_avg,
        "bb_win_avg": bb_win_avg, "bb_loss_avg": bb_loss_avg,
        "current_params": _sw_params.copy(),
    }

# ── CLAUDE LERN-AGENT ──────────────────────────────────────────────────────────
def sideways_claude_agent():
    """
    Alle 4h: Claude analysiert Sideways-Trades und empfiehlt Parameter.
    Angepasst werden: min_score, tp_pct, stop_pct, range_buy_pct.
    """
    global _sw_last_claude, _sw_params
    now = time.time()
    if now - _sw_last_claude < _sw_claude_interval:
        return
    _sw_last_claude = now

    stats = get_sideways_stats()
    if not stats or stats["total"] < 5:
        log(f"SW AGENT: zu wenig Daten ({stats['total'] if stats else 0} Trades)")
        return

    api_key = os.getenv("ANTHROPIC_API_KEY", "")
    if not api_key:
        # Regelbasierter Fallback ohne Claude API
        _sw_rule_based_adjust(stats)
        return

    try:
        prompt = f"""Du bist ein Crypto-Trading-Bot-Optimierer für SIDEWAYS-Märkte.

Aktuelle Sideways-Performance:
- Trades: {stats['total']} | WinRate: {stats['wr']:.0%}
- Ø Gewinn: ${stats['avg_win']:.4f} | Ø Verlust: ${stats['avg_loss']:.4f}
- Range-Trades: {stats['range_trades']} | Range-WinRate: {stats['range_wr']:.0%}
- RSI bei Gewinnen: {stats['rsi_win_avg']:.0f} | RSI bei Verlusten: {stats['rsi_loss_avg']:.0f}
- BB-Breite bei Gewinnen: {stats['bb_win_avg']:.3f} | bei Verlusten: {stats['bb_loss_avg']:.3f}

Aktuelle Parameter:
{json.dumps(stats['current_params'], indent=2)}

Analysiere die Daten und empfehle optimierte Parameter für den Sideways-Markt.
Antworte NUR mit JSON, kein Text davor oder danach:
{{
  "min_score": <float 7.0-15.0>,
  "tp_pct": <float 0.02-0.08>,
  "stop_pct": <float 0.008-0.025>,
  "range_buy_pct": <float 0.01-0.05>,
        ...
        if resp.status_code == 200:
            ...
            if s >= 0:
                ...
                limits = {
                    "min_score":     (7.0,  15.0),
                    "tp_pct":        (0.02, 0.08),
                    "stop_pct":      (0.008,0.025),
                    "range_buy_pct": (0.01, 0.05),
                }
                for k, v in rec.items():
                    if k in _sw_params and k in limits:
                        lo, hi = limits[k]
                        new_v = round(max(lo, min(hi, float(v))), 4)
                        if abs(new_v - _sw_params[k]) > 0.001:
                            changed.append(f"{k}: {_sw_params[k]:.4f}→{new_v:.4f}")
                            _sw_params[k] = new_v
                if changed:
                    log(f"SW AGENT: {', '.join(changed)}")
                    log(f"SW AGENT Begründung: {reasoning}")
                    save_sw_params()
                else:
                    log(f"SW AGENT: keine Parameteränderung nötig | {reasoning}")
    except Exception as e:
        log(f"SW AGENT ERR: {e}")
        _sw_rule_based_adjust(stats)

def _sw_rule_based_adjust(stats):
    """Regelbasierte Anpassung falls kein Claude API Key."""
    global _sw_params
    changed = []

    # WinRate < 40%: Score anheben (strengerer Filter)
    if stats["wr"] < 0.40 and _sw_params["min_score"] < 13.0:
        _sw_params["min_score"] = round(_sw_params["min_score"] + 0.5, 1)
        changed.append(f"min_score↑{_sw_params['min_score']}")

    # WinRate > 65% und genug Trades: Score etwas lockern für mehr Trades
    elif stats["wr"] > 0.65 and stats["total"] > 15 and _sw_params["min_score"] > 8.0:
        _sw_params["min_score"] = round(_sw_params["min_score"] - 0.3, 1)
        changed.append(f"min_score↓{_sw_params['min_score']}")

    # Range-Trading läuft gut: Zone erweitern
    if stats["range_wr"] > 0.60 and stats["range_trades"] >= 3:
        _sw_params["range_buy_pct"] = min(0.05, _sw_params["range_buy_pct"] + 0.005)
        changed.append(f"range_buy↑{_sw_params['range_buy_pct']:.3f}")

    # Verluste im Sideways zu groß: Stop enger
    if stats["avg_loss"] and stats["avg_loss"] < -0.10:
        _sw_params["stop_pct"] = max(0.008, _sw_params["stop_pct"] - 0.001)
        changed.append(f"stop↓{_sw_params['stop_pct']:.3f}")

    if changed:
        log(f"SW REGEL-ANPASSUNG: {', '.join(changed)}")
        save_sw_params()
    else:
        log(f"SW REGEL: keine Änderung | WR={stats['wr']:.0%} Trades={stats['total']}")
...
def intx_request(method, path, body=None):
    ...
    try:
        ...
        if r.status_code in [200, 201]:
            reset_ip_block()
            return r.json()
        if r.status_code == 403 and handle_ip_block(r.text):
            return None  # IP-Block erkannt, Backoff aktiv
...
def run():
    ...
    while True:
        try:
            ...
            # SIDEWAYS LERN-AGENT alle 4h
            if cycle % 1440 == 0:
                threading.Thread(target=sideways_claude_agent, daemon=True).start()
                ...
                if reason:
                    ...
                    if market_sell(client, pair, sell_size if reason=="PARTIAL_TP" else size):
                        ...
                        record_trade_ki(
                            ...
                            btc=btc_t)
                        # SIDEWAYS LERNEN: Trade mit Kontext speichern
                        try:
                            _data_sw = _candle_cache.get(pair)
                            _sw_d = _data_sw[1] if _data_sw else {}
                            _sw_closes = _sw_d.get("close", [])
                            _sw_rsi = rsi(_sw_closes) or 50 if _sw_closes else 50
                            _, _, _sw_bbw, _ = bollinger(_sw_closes) if len(_sw_closes) >= 20 else (0,0,0,0)
                            _sw_r_low, _sw_r_high, _, _sw_in_range = detect_range(_sw_d, pair) if _sw_d else (0,0,0,False)
                            save_sideways_trade(
                                pair=pair, entry=entry, exit_price=bid,
                                pnl_pct=pnl, profit_usdc=profit_usdc,
                                phase=phase, strategy="range" if _sw_in_range else "trend",
                                rsi_entry=_sw_rsi, bb_width=_sw_bbw or 0,
                                range_low=_sw_r_low, range_high=_sw_r_high,
                                was_range_trade=1 if _sw_in_range else 0)
                        except Exception as _sw_e:
                            log(f"SW RECORD ERR: {_sw_e}")
                        if pnl > 0:
                            wins += 1
                            consecutive_losses = 0
                        else:
            ...
            if can_buy and scan_now:
                ...
                for pair in scan_pairs:
                    ...
                    # SIDEWAYS RANGE-TRADE: Bonus wenn Preis nahe Range-Low
                    range_buy_ok, r_low, r_high = is_range_buy(data, pair, phase)
                    if phase == "sideways" and range_buy_ok:
                        score += 3.0  # Range-Entry Bonus

                    sw_min = _sw_params["min_score"] if phase == "sideways" else MIN_SCORE
                    if score >= sw_min:
        ...
        # IP-Block? Dann warten statt normal schlafen
        if is_ip_blocked():
            wait_if_ip_blocked()
        else:
